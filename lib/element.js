...
class Element extends Emitter {
	...
	getSize(prop) {
		var size = {
			width: toInt(moofx(this.dom).compute('width').replace('px', '')),
			height: toInt(moofx(this.dom).compute('height').replace('px', ''))
		}
	...
	getCoord(prop) {
		...
		var coordinate = {
			top: toInt(moofx(this.dom).compute('top').replace('px', '')),
			left: toInt(moofx(this.dom).compute('left').replace('px', '')),
			bottom: toInt(moofx(this.dom).compute('bottom').replace('px', '')),
			right: toInt(moofx(this.dom).compute('right').replace('px', ''))
		}
